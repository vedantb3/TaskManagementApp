...
var ln = function(t2) {
  const e2 = t2.clipboardData;
  if (e2) return e2.types.includes("Files") && 1 === e2.types.length && e2.files.length >= 1;
};
var cn = function(t2) {
  const e2 = t2.clipboardData;
    ...
    if (date instanceof Date)
      ...
      if (datestr === "today") {
        ...
        ops.forEach(function(_a2) {
          ...
          return parsedDate = fn3(parsedDate, val, locale) || parsedDate;
        });
...
document.addEventListener("DOMContentLoaded", function() {
  esm_default("#deadline-picker", {
    enableTime: true,
    // Enable time selection as well
    dateFormat: "Y-m-d H:i",
    // Format for the date and time
    minDate: "today",
    // Minimum selectable date is today
    defaultHour: 12
    // Default hour to show in the time picker
  });
});
